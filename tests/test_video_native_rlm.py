...
def toolkit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, fake_video: Path) -> VideoToolkit:
    ...
    tk._llm_client = DummyMediaLLM(["frame result"], model="google-vertex:gemini-3.1-pro-preview")
...
def test_zero_length_video_inspection_becomes_single_frame(toolkit: VideoToolkit):
    ...
    result = inspect(prompt="Describe the frame", start_s=12.0, end_s=12.0)

    assert result == "frame result"
    llm = toolkit._llm_client
    assert llm.calls[0]["media"][0]["media_type"] == "image/jpeg"
...
def test_agent_defaults_and_api_contract():
    ...
    assert agent.sub_model == "google-vertex:gemini-3.1-pro-preview"
